...
enum _:UserDataStruct {
	...
	// Undo gocheck data
	bool:ud_CanUndoGoCheck,
	Float:ud_UndoGoCheckPos[3],
	Float:ud_UndoGoCheckAngle[3],
...
initCommands() {
	...
	kz_register_cmd("ungocheck", 	"cmd_UndoGoCheck");
...
public plugin_natives()
{
	...
	register_native("kz_can_undo_gocheck", "native_can_undo_gocheck");
}
...
public native_can_undo_gocheck() {
	new id = get_param(1);

	return g_UserData[id][ud_CanUndoGoCheck];
}
...
public cmd_Checkpoint(id) {
	...
	// 检查是否在地面上或是否在滑坡
	new bool:bIsOnGround = (get_entvar(id, var_flags) & FL_ONGROUND) || isUserSurfing(id);
	
	// air check - 修改为：不在地面且不在滑坡，且移动类型不是飞行
	if (!bIsOnGround && (get_entvar(id, var_movetype) != MOVETYPE_FLY)) {
...
public cmd_Gocheck(id) {
	...
	static i;
	static Float:vUndoPos[3], Float:vUndoAngle[3];
	new bool:bCanSaveUndo = (get_entvar(id, var_flags) & FL_ONGROUND) != 0;

	if (bCanSaveUndo) {
		get_entvar(id, var_origin, vUndoPos);
		get_entvar(id, var_v_angle, vUndoAngle);
	}
	...
	if (bCanSaveUndo) {
		g_UserData[id][ud_CanUndoGoCheck] = true;
		g_UserData[id][ud_UndoGoCheckPos] = vUndoPos;
		g_UserData[id][ud_UndoGoCheckAngle] = vUndoAngle;
	} else {
		g_UserData[id][ud_CanUndoGoCheck] = false;
	}
...
public cmd_UndoGoCheck(id) {
	if (!is_user_alive(id)) return PLUGIN_HANDLED;
	if (!g_UserData[id][ud_CanUndoGoCheck]) return PLUGIN_HANDLED;

	set_entvar(id, var_origin, g_UserData[id][ud_UndoGoCheckPos]);

	if (g_UserData[id][ud_AnglesMode] & (1 << 0)) {
		set_entvar(id, var_angles, g_UserData[id][ud_UndoGoCheckAngle]);
		set_entvar(id, var_v_angle, g_UserData[id][ud_UndoGoCheckAngle]);
		set_entvar(id, var_fixangle, 1);
	}
	...
	if (g_UserData[id][ud_TimerState] != TIMER_PAUSED && g_UserData[id][ud_TeleNum] > 0)
		g_UserData[id][ud_TeleNum]--;
...
public cmd_Fade(id) {
	...
	if (g_UserData[id][ud_Sunglasses]) {
...
stock bool:isUserSurfing(id) {
	...
	engfunc(EngFunc_TraceHull, origin, dest, 0, 
		(get_entvar(id, var_flags) & FL_DUCKING) ? HULL_HEAD : HULL_HUMAN, id, 0);
